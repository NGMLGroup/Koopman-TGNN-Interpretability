...
class ESN(object):
    ...
    def _fit_transform(self, Xtr, Ytr, n_drop = 100, regression_method = 'linear', regression_parameters = None, embedding = 'identity', n_dim = 3, embedding_parameters = None):
        ...
        if (self._input_weights is None):
            self._input_weights = 2.0*torch.rand(self._n_internal_units, dim_data, device=self._device) - 1.0

        if (self._feedback_weights is None):
            self._feedback_weights = 2.0*torch.rand(self._n_internal_units, dim_output, device=self._device) - 1.0
    ...
    def _compute_state_matrix(self, X, Y = None, n_drop = 100):
        ...
        # Initial values
        previous_state = torch.zeros((1, self._n_internal_units), dtype=float, device=self._device)
        previous_output = torch.zeros((1, self._dim_output), dtype=float, device=self._device)

        # Storage
        state_matrix = torch.empty((n_data - n_drop, self._n_internal_units), dtype=float, device=self._device)
        embedded_states = torch.empty((n_data - n_drop, self._embedding_dimensions), dtype=float, device=self._device)
        outputs = torch.empty((n_data - n_drop, self._dim_output), dtype=float, device=self._device)

        for i in range(n_data):
            # Process inputs
            previous_state = torch.atleast_2d(previous_state).to(self._device)
            current_input = torch.atleast_2d(self._scaleshift(X[i, :], self._input_scaling, self._input_shift)).to(self._device)
            feedback = self._feedback_scaling*torch.atleast_2d(previous_output).to(self._device)
    ...
    def _initialize_internal_weights(self, n_internal_units, connectivity, spectral_radius):
        ...
        return torch.from_numpy(np.array(internal_weights)).to(self._device)
